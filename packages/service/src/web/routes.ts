...
import type { EventBroker, EventSubscription } from "../policy/event-broker"
import type { OperationRegistry, WorkspaceCreateMutation } from "../policy/operations"
import type { Signal, SignalDismissal, WorkspaceCreationCatalog } from "@git-stacks/protocol"
import type { ServiceRequestContext, TerminalSocket } from "../carrier"
...
import { WebTerminalManager } from "./terminal-manager"
...
type Mutation = (request: { workspace: string; options?: Record<string, unknown> }, signal?: AbortSignal) => import("../policy/operations").OperationExecution
  ...
  stream(): ReadableStream<Uint8Array> {
    ...
    return new ReadableStream<Uint8Array>({
      start: async (controller) => {
        try {
          while (!this.closed) {
            if (!this.subscription.peek()) await Promise.race([
              this.subscription.waitForAvailable(),
              new Promise<void>((resolve) => setTimeout(resolve, this.heartbeatMs)),
            ])
...
export class WebApplication {
  ...
  async handle(request: Request, server: ServiceRequestContext): Promise<Response | undefined> {
  ...
  websocket = {
    open: (socket: TerminalSocket) => { this.terminals.attach(socket); this.notifyConnections() },
    message: (socket: TerminalSocket, message: string | Buffer) => this.terminals.message(socket, message),
    close: (socket: TerminalSocket) => { this.terminals.detached(socket); this.notifyConnections() },
    drain: (socket: TerminalSocket) => this.terminals.drain(socket),
  }
  ...
  private async api(request: Request, url: URL, principal: WebPrincipal, server: ServiceRequestContext): Promise<Response> {
    if (request.method === "GET" && url.pathname === "/web/api/discovery") return webJson({
      service_version: "1",
      capabilities: {
        snapshots: true,
        terminals: Boolean(this.options.snapshot.resolveTerminalLaunch),
  ...
  private async submitOperation(request: Request, principal: WebPrincipal): Promise<Response> {
    ...
    let execution: import("../policy/operations").OperationExecution
  ...
  private async events(url: URL, request: Request, principal: WebPrincipal, server: ServiceRequestContext): Promise<Response> {
  ...
  private asset(pathname: string): Response {
    ...
    return new Response(readFileSync(join(this.options.assetsRoot, name)), { headers: securityHeaders(contentType) })
  }
