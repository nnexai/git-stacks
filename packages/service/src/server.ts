...
import { authenticateAdmission, type AuthenticatedClient } from "./policy/credentials"
import { IdempotencyConflictError, type CoreMutationAdapter, type OperationRegistry, type OperationExecution } from "./policy/operations"
import type { EventBroker, EventReservation, EventSubscription, EventSubscriptionDiagnostics } from "./policy/event-broker"
import { SignalDismissalSchema, SignalSchema, type Signal, type SignalDismissal } from "@git-stacks/protocol"
import { CLIENT_MODEL_LIMITS, WorkspaceCreationRequestSchema, type WorkspaceCreationCatalog } from "@git-stacks/protocol"
import type { WorkspaceCreateMutation } from "./policy/operations"
import { SnapshotBusyError } from "./policy/snapshot"
import type { WebApplication } from "./web/routes"
import type { ServiceRequestContext } from "./carrier"
import { startNodeCarrier, type NodeCarrierServer, type NodeWebSocketHandlers } from "./node-server"
import type { SnapshotAdapter } from "./snapshot-adapter"
import { EditTargetRequestSchema, CoreMutationSchemas, type CoreMutationName } from "./policy/core-contract"
import type { CoreStateProvider } from "./policy/core-state"
        ...
        try {
          ...
          while (!this.released) {
            ...
            if (this.subscription.disconnect) {
              controller.enqueue(encoder.encode(`event: disconnect\ndata: ${JSON.stringify(this.subscription.disconnect)}\n\n`))
            }
            controller.close()
            this.release()
            return
...
export async function startServiceServer(options: ServiceServerOptions): Promise<RunningServiceServer> {
  ...
  const events = async (request: Request, client: AuthenticatedClient, id: string, server: ServiceRequestContext, finite = false): Promise<Response> => {
  ...
  const handle = async (request: Request, server: ServiceRequestContext): Promise<Response | undefined> => {
  ...
  const websocket: NodeWebSocketHandlers = {
  ...
  const server = await startNodeCarrier({
