...
export type WorkspaceCreationCatalog = {
  templates: Array<{ name: string; description?: string; repository_count: number; command_count: number; labels: string[] }>
  repositories: Array<{ name: string; type: RepoRegistryEntry["type"]; default_branch: string }>
}
...
async function validateBranchWithGit(branch: string): Promise<boolean> {
  const proc = spawn(["git", "check-ref-format", "--branch", branch], {
    stdout: "ignore",
    stderr: "ignore",
  })
