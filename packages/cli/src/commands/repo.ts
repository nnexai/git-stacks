...
import { readRegistry, updateRegistry, readGlobalConfig, NameSchema, type RepoRegistryEntry } from "@git-stacks/core/config"
import { expandHome } from "@git-stacks/core/paths"
import { detectRepoType } from "@git-stacks/core/detect"
import { getCurrentBranch } from "@git-stacks/core/git"
import { runRepoScan } from "../wizards/repo-wizard"
import { prompts as p } from "../prompts"
import { editRegistryYaml, openYamlInEditor } from "@git-stacks/core/workspace-yaml"
import { detectForgeForRepoEnabled } from "@git-stacks/core/integrations/forge-utils"
...
repoCommand
  ...
  .action(async (rawPath: string, opts: { name?: string; branch?: string }) => {
    ...
    updateRegistry((current) => {
      if (current.some((candidate) => candidate.name === entry.name)) throw new Error(`Repo '${entry.name}' already exists in registry.`)
...
repoCommand
  ...
  .action(async (repo: string, opts: { force?: boolean }) => {
    ...
    updateRegistry((current) => current.filter((entry) => entry.name !== repo))
...
repoCommand
  ...
  .action((repo: string, newName: string) => {
    ...
    updateRegistry((current) => {
      if (!current.some((entry) => entry.name === repo)) throw new Error(`Repo '${repo}' not found in registry.`)
      if (current.some((entry) => entry.name === newName)) throw new Error(`Repo '${newName}' already exists in registry.`)
      return current.map((entry) => entry.name === repo ? { ...entry, name: newName } : entry)
    })
