...
import { readTemplate, writeTemplate, updateTemplate, listTemplates, templateExists, deleteTemplate } from "@git-stacks/core/config"
import { runTemplateNew, runTemplateEdit } from "../wizards/template-wizard"
import { editTemplateYaml, openYamlInEditor } from "@git-stacks/core/workspace-yaml"
import { renameTemplate } from "@git-stacks/core/workspace-ops"
...
templateLabelCommand
  ...
  .action((template: string, labels: string[]) => {
    ...
    const next = updateTemplate(template, (tpl) => ({ ...tpl, labels: [...new Set([...(tpl.labels ?? []), ...labels])] }))
...
templateLabelCommand
  ...
  .action((template: string, labels: string[]) => {
    requireTemplate(template)
    labels.forEach(validateLabel)
    const next = updateTemplate(template, (tpl) => {
      const filtered = (tpl.labels ?? []).filter(label => !labels.includes(label))
      return { ...tpl, labels: filtered.length > 0 ? filtered : undefined }
    })
...
templateLabelCommand
  ...
  .action((template: string) => {
    requireTemplate(template)
    updateTemplate(template, (tpl) => ({ ...tpl, labels: undefined }))
