...
import { $, spawnSync } from "@git-stacks/core/node-runtime"
...
import { resolveEnabledGlobally } from "@git-stacks/core/integrations/types"
import { getTasksDir, WORKSPACES_DIR, TEMPLATES_DIR } from "@git-stacks/core/paths"
  ...
  switch (fix.action) {
    ...
    case "open-workspace": {
      const result = spawnSync(
        [process.execPath, process.argv[1], "open", fix.name, "--no-ide", "--no-cmux"],
        { stdio }
      )
    ...
    case "remove-repo": {
      const result = spawnSync(
        [process.execPath, process.argv[1], "repo", "remove", fix.name, "--force"],
        { stdio }
      )
      return result.exitCode === 0 ? { ok: true } : { ok: false, error: `exit ${result.exitCode}` }
    }
