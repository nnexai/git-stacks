...
pub const TerminalRuntime = struct {
    ...
    pub fn resizeViewport(self: *TerminalRuntime, columns: u16, rows: u16, width: u16, height: u16) !void { try self.pty.resizeViewport(columns, rows, width, height); try self.terminal.resize(columns, rows); }
