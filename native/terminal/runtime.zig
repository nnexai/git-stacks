...
pub const TerminalRuntime = struct {
    ...
    pub fn close(self: *TerminalRuntime) void { if (!self.live) return; self.live = false; self.generation +%= 1; self.pty.terminate(); _ = self.pty.wait() catch 0; self.pty.close(); self.terminal.deinit(); counters.pty_fds -= 1; counters.children -= 1; counters.groups -= 1; }
};
