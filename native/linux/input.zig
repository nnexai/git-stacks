...
pub const Input = struct {
    ...
    pub fn paste(self: *Input, text: []const u8, confirmed: bool) !void { try self.active(); if (text.len > 1024 * 1024) return error.InputTooLarge; if (!self.runtime.terminal.safePaste(text) and !confirmed) return error.UnsafePaste; const encoded = try self.runtime.terminal.encodePaste(self.runtime.allocator, text); defer self.runtime.allocator.free(encoded); try self.runtime.send(encoded); }
