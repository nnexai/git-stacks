...
fn valid(self:*const Launcher,c:model.Command)bool{return !std.mem.eql(u8,c.name[0..c.name_len],"tui-output-smoke") and std.mem.eql(u8,&c.workspace_id,&self.pair.workspace_id) and (c.repository_id==null or std.mem.eql(u8,&c.repository_id.?,&self.pair.repository_id));}
