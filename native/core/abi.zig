...
export fn gs_native_abi_version_v1() u32 {
    return 1;
}

fn clear(out: ?*Bytes) void {
...
fn copyString(comptime N: usize, target: *[N]u8, source: []const u8) ?usize {
    if (source.len > N or !std.unicode.utf8ValidateSlice(source)) return null;
...
fn parseAttention(object: std.json.ObjectMap) ?state_model.Attention {
    ...
    if (object.get("detail")) |v| {
        if (v != .string) return null;
        item.detail_len = @intCast(copyString(500, &item.detail, v.string) orelse return null);
    }
    if (object.get("occurred_at")) |v| {
        if (v != .string) return null;
...
fn parseSnapshot(object: std.json.ObjectMap) ?state_model.State {
    ...
    for (pairs.array.items) |entry| {
        ...
        for (surfaces.array.items) |surface_value| {
            ...
            if (so.get("kind")) |v| {
                if (v != .string) return null;
                surface.kind = if (std.mem.eql(u8, v.string, "configured_command")) .configured_command else if (std.mem.eql(u8, v.string, "shell")) .shell else return null;
            }
            if (so.get("command_id")) |v| {
                if (v != .string) return null;
                surface.command_id_len = @intCast(copyString(64, &surface.command_id, v.string) orelse return null);
            }
            ...
            if (so.get("title")) |title| {
                if (title != .string) return null;
                surface.title_len = @intCast(copyString(64, &surface.title, title.string) orelse return null);
            }
            if (so.get("cwd")) |cwd| {
                if (cwd != .string) return null;
                surface.cwd_len = @intCast(copyString(128, &surface.cwd, cwd.string) orelse return null);
            }
...
export fn gs_model_create_v1(version: u32, input: Bytes, out_model: ?*?*Model, out_error: ?*Bytes) c_int {
    ...
    const copy = allocator.dupe(u8, bytes) catch {
        ...
        return fail(out_of_memory, "internal_error", out_error);
    };
    model.* = .{ .json = copy };
    target.* = model;
    return ok;
...
export fn gs_model_dispatch_v1(model: ?*Model, input: Bytes, out_json: ?*Bytes, out_error: ?*Bytes) c_int {
    ...
    const action: reducer.Action = if (std.mem.eql(u8, kind.string, "snapshot")) .{ .snapshot = parseSnapshot(object) orelse return fail(invalid_json, "invalid_request", out_error) } else if (std.mem.eql(u8, kind.string, "attention_received")) .{ .attention_received = parseAttention(object) orelse return fail(invalid_json, "invalid_request", out_error) } else if (std.mem.eql(u8, kind.string, "disconnected")) .disconnected else if (std.mem.eql(u8, kind.string, "unknown_optional")) .unknown_optional else if (std.mem.eql(u8, kind.string, "incompatible")) .incompatible else if (std.mem.eql(u8, kind.string, "capacity_exceeded")) .{ .capacity_exceeded = .{ .kind = blk: {
        ...
        return fail(invalid_json, "invalid_request", out_error);
    }, .maximum = jsonU32(object, "maximum", 0) orelse return fail(invalid_json, "invalid_request", out_error), .attempted = jsonU32(object, "attempted", 0) orelse return fail(invalid_json, "invalid_request", out_error) } } else if (std.mem.eql(u8, kind.string, "connected")) .{ .connected = .{ .revision = jsonU64(object, "revision") orelse return fail(invalid_json, "invalid_request", out_error), .sequence = jsonU64(object, "sequence") orelse return fail(invalid_json, "invalid_request", out_error) } } else if (std.mem.eql(u8, kind.string, "event")) .{ .event = .{ .revision = jsonU64(object, "revision") orelse return fail(invalid_json, "invalid_request", out_error), .sequence = jsonU64(object, "sequence") orelse return fail(invalid_json, "invalid_request", out_error) } } else if (std.mem.eql(u8, kind.string, "refreshed")) .{ .refreshed = .{ .revision = jsonU64(object, "revision") orelse return fail(invalid_json, "invalid_request", out_error), .sequence = jsonU64(object, "sequence") orelse return fail(invalid_json, "invalid_request", out_error) } } else if (std.mem.eql(u8, kind.string, "select_attention")) .{ .select_attention = .{ .attention_id = jsonId(object, "attention_id") orelse return fail(invalid_identity, "invalid_request", out_error) } } else if (std.mem.eql(u8, kind.string, "exact_tab_visible")) .{ .exact_tab_visible = .{ .surface_id = jsonId(object, "surface_id") orelse return fail(invalid_identity, "invalid_request", out_error) } } else if (std.mem.eql(u8, kind.string, "remove_attention")) .{ .remove_attention = .{ .attention_id = jsonId(object, "attention_id") orelse return fail(invalid_identity, "invalid_request", out_error) } } else if (std.mem.eql(u8, kind.string, "navigate_pair")) .{ .navigate_pair = .{ .workspace_id = jsonId(object, "workspace_id") orelse return fail(invalid_identity, "invalid_request", out_error), .repository_id = jsonId(object, "repository_id") orelse return fail(invalid_identity, "invalid_request", out_error) } } else return fail(invalid_json, "invalid_request", out_error);
