...
import { startManagedService } from "../service/main"
...
export const webCommand = new Command("web")
  ...
  .action(async (options: { open: boolean; json?: boolean }) => {
    const manifest = join(import.meta.dir, "../../dist/web/manifest.json")
    if (!existsSync(manifest)) {
      try { await import("../../scripts/build-web") } catch { throw new Error("Web assets are missing; run 'bun run web:build'") }
    }
