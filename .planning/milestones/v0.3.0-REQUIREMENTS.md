# Requirements Archive: v0.3.0 Dashboard UI Overhaul
...
- [x] **MSG-01**: User can send a notification to a workspace via `git-stacks message send "<text>"` with workspace auto-detected from the `WS_WORKSPACE` environment variable
- [x] **MSG-02**: User can explicitly specify the target workspace via `--workspace <name>` in `message send`, `message list`, and `message clear`
- [x] **MSG-03**: User can optionally identify the message sender via `--from <sender-name>` in `message send`
- [x] **MSG-04**: User can list all active notifications for a workspace via `git-stacks message list [--workspace <name>]`, showing sender, text, and timestamp
- [x] **MSG-05**: User can clear all notifications for a workspace via `git-stacks message clear [--workspace <name>]`
- [x] **MSG-06**: User can clear notifications from a specific sender via `git-stacks message clear --from <sender-name>`
- [x] **MSG-07**: Each notification is persisted to a JSONL file at `~/.config/git-stacks/messages/{workspace}.jsonl`; messages survive TUI restarts
- [x] **MSG-08**: `git-stacks message send` exits 0 when the TUI is not running (IPC push silently dropped; file write still succeeds)
- [x] **MSG-09**: When the TUI is running, `git-stacks message send` delivers the notification in real-time via Unix socket without requiring a manual `R` refresh
- [x] **MSG-10**: TUI opens a Unix socket on startup and removes the socket file on clean exit; stale socket from a previous crash is detected and replaced at startup
...
| MSG-05 | Phase 6 | Complete |
| MSG-06 | Phase 6 | Complete |
| MSG-07 | Phase 6 | Complete |
| MSG-08 | Phase 6 | Complete |
| MSG-09 | Phase 6 | Complete |
| MSG-10 | Phase 6 | Complete |
