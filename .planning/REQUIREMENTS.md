...
- [ ] **FSYNC-01**: User can define `files.sync` entries in templates and workspaces with `source`, `target`, and optional `git_exclude` fields.
- [ ] **FSYNC-02**: User can pull a sync source into a workspace target as real files without using symlinks.
...
- [ ] **FSYNC-07**: Sync push detects obvious conflicts and refuses unsafe writes by default without requiring a full per-file hash manifest for large trees.
- [ ] **FSYNC-08**: Sync delete/overwrite behavior is conservative by default and requires explicit flags or documented policy before removing source-side files.
...
- [ ] **FSRC-02**: GitLab merge request URLs are researched and resolved first through the enabled GitLab integration, with documented `glab` limitations and validation constraints.
- [ ] **FSRC-03**: Forge source resolution matches the source target repository to a repo in the selected template using existing registry/forge/upstream metadata where possible.
...
- [ ] **FSRC-08**: Gitea and GitHub source support are designed after GitLab, with at least URL parsing and resolver contract tests even if full live validation is deferred.

### Documentation and Release

- [ ] **DOCS-01**: README documents `files.sync`, `git-stacks files status|pull|push`, local exclude behavior, and the manual push-back model.
...
|-------------|-------|--------|
| FSYNC-01 | Phase 89 | Pending |
| FSYNC-02 | Phase 89 | Pending |
| FSYNC-03 | Phase 89 | Pending |
| FSYNC-04 | Phase 90 | Pending |
| FSYNC-05 | Phase 90 | Pending |
| FSYNC-06 | Phase 90 | Pending |
| FSYNC-07 | Phase 90 | Pending |
| FSYNC-08 | Phase 90 | Pending |
