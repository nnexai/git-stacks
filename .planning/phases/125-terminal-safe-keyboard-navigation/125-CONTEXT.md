...
- Rebinding is an explicit capture flow. Each action has one primary binding plus bounded optional aliases. Conflicting active primary or alias bindings are rejected atomically with the other action named; every action can be unbound and reset to its platform default. Reset restores the safe primary and removes aliases, while Unbind clears primary and aliases. Optional `Ctrl+K` and `Ctrl+Shift+P` aliases begin absent and can be added without replacing the safe primary.
