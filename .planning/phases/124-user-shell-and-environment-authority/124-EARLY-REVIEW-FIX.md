...
- Bash and zsh execute every authoritative expansion, equality check, and readiness write inside a grammar-level block whose stdout and stderr are redirected. Bash first moves `BASH_XTRACEFD` to fd 2 and verifies the assignment before entering that block, so a profile-selected alternate trace descriptor cannot bypass the boundary. The disposable command shell retains fd 2 as its trace target: attempting to restore a profile-owned descriptor could close stderr or select a descriptor already closed by Bash. A readonly trace descriptor fails closed because readiness is never created.
...
- Captured stdout-plus-stderr fixtures enable Bash xtrace on a dedicated fd and fish tracing while retaining the hostile dispatcher functions. They assert that command stdout, command stderr, and readiness survive while the raw authoritative sentinel is absent. The Bash assertion specifically proves stderr remains usable after replacing the custom trace fd. The same regression runs for zsh when the executable is installed; local zsh remains the single explicit capability skip.
