...
- 🚧 **v0.15.0 Dir Mode & Polish** — Phases 64-68 (in progress)
...
**Goal**: Users can declare non-git directories as "dir" repos in registry and template YAML, and add or scan them via CLI
**Depends on**: Phase 63 (v0.14.0 complete)
**Requirements**: SCHM-01, SCHM-02, REG-01, REG-02
**Success Criteria** (what must be TRUE):
  1. User can set `type: dir` on a repo in registry YAML and have it validated by Zod without error
...
**Success Criteria** (what must be TRUE):
  1. `git-stacks push` on a workspace with dir repos completes with only non-dir repos pushed; dir repos are silently skipped
...
**Requirements**: DISP-01, DISP-02, DISP-03, HLTH-01, HLTH-02
**Success Criteria** (what must be TRUE):
  1. `git-stacks status` shows dir repos with a "dir" label and no ahead/behind/dirty columns
...
**Depends on**: Phase 67
**Requirements**: (none — release process)
**Success Criteria** (what must be TRUE):
