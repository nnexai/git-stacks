...
describe("signal state", () => {
  ...
  test("completed activity and notifications independently require attention", () => {
    const state = new SignalState()
    ...
    expect(state.projection().unread.map((item) => item.id)).toEqual([base.id, "sig_2123456789abcdef"])
    state.apply({ sequence: "3", dismissal: { kind: "dismiss_signal", signal_id: "sig_2123456789abcdef" } })
    expect(state.projection().unread).toMatchObject([{ id: base.id, state: "completed" }])
