...
describe("web canonical workspace action surface", () => {
  ...
  test("uses stable rename intent and production-wired overlay focus coordination", () => {
    ...
    expect(appSource).toContain("createWebOverlayRuntime")
    ...
    expect(appSource).toContain('row.setAttribute("data-workspace-placement", placementId)')
    expect(appSource).toContain("findWorkspaceRow(document, workspace.id, repository.id, placementId)")
    expect(appSource).toContain("terminal.focusInput(attempt)")
    expect(appSource).toContain("bindInPlaceOverlayRetry(retry, reload)")
    expect(appSource).not.toContain('retry.addEventListener("click", () => { void reload() })')
