...
async function verifyQuick(): Promise<void> {
  ...
  await verifyModel()
  await verify("service-client-test")
...
else if (mode === "service-sync") await verify("service-sync-test")
else if (mode === "workspace-creation") await verify("workspace-creation-test")
