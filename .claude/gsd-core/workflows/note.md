...
- **Global scope**: `/home/nnex/dev/prj/git-stacks/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/home/nnex/dev/prj/git-stacks/.claude/notes/`)
...
2. Glob `/home/nnex/dev/prj/git-stacks/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/home/nnex/dev/prj/git-stacks/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/home/nnex/dev/prj/git-stacks/.claude/notes/` — works in any directory
